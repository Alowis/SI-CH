...
#TO DO: Create a data frame with the area of each grid of the domain
...
metavHour<-list()
for(hazard in 1:3){
if(hazard==1){hazdat=hazmat$Pr;th=th1}
if(hazard==2){hazdat=hazmat$Wg;th=th2}
...
if (hazard == 3){
  ...
  {
  bolilos<-hazdat$data  
  bolilos[hazdat$data<th] <- NA
  bolilos[hazdat$data>=th] <- 1
  if(hazard==1){formeta[hazdat$data<th] <- NA}
  if(hazard==2){formeta2[hazdat$data<th] <- NA}
}
...
rbPal2<-colorRampPalette(c('white',"blue","red","purple"))
#Hotspot locations
ggplot(uk_fort, aes(x=long,y=lat,group=group)) +
  geom_polygon(fill = "white", color = "gray10", size = 1) +
  theme_bw(16)+
...
ggplot(uk_fort, aes(x=long,y=lat,group=group)) +
  ...
  # geom_bin2d(data=metaHaz[[2]],aes(x=lonMW,y=latMW,group=season),alpha=.7,binwidth=c(.5,.5))+
  # geom_density2d(data=metaHaz[[2]],aes(x=lonMW,y=latMW,group=season),alpha=.2)
  stat_density_2d(data=metaHaz[[2]],aes(x=lonMW,y=latMW,group=season,fill = stat(density)),geom = "raster", contour = FALSE,alpha=.6,n=150)
...
polarEvent<-aggregate(metaHaz[[1]]$season,
                 by = list(Month = metaHaz[[1]]$x.month,Year=metaHaz[[1]]$x.year),
...
plot(metaHaz[[1]]$rf,metaHaz[[1]]$x.dur,col=alpha(1,0.6),pch=16,log="xy", xlim=c(1,40000),ylim=c(2.9,120),cex= round(metaHaz[[1]]$rf.max/mean(metaHaz[[1]]$rf.max),4))
points(doubleExRevent$rf,doubleExRevent$x.dur,col=alpha(2,.5),pch=16, xlim=c(1,40000),ylim=c(2.9,120),cex= round(doubleExRevent$wg.max/min(doubleExRevent$wg.max),4))
...
##############################Temporal lag between events###########
  ...
  for (haz in 1:3){
    ...
    evclust<-c(evclust,list(eventid))
  }
  names(totdates)=c("Pr-Events","Wind_Events","Compound")
  plot(totdates[[1]][,1])
  points(totdates[[3]][,1],col=2,pch=3)
  points(totdates[[2]][,1],col=3,pch=4) 
  teslag<-data.frame(totdates[[2]],evclust[[2]])
  teslag2<-data.frame(totdates[[1]],evclust[[1]])
  
  
  
###Temporal coincindence start (as i want to go to the hour level)####

  teslag2<-teslag2[order(teslag2[,1]),]
  teslag<-teslag[order(teslag[,1]),]
  
  teslagx[,1]<-as.Date(teslag[,1])
  teslagy[,1]<-as.Date(teslag2[,1])
matcev<-match(teslagx[,1],teslagy[,1])
matcev<-na.omit(matcev)
rainmatch<-teslag[matcev,]
