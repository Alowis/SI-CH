...
save(nwo,file="windallclusters.Rdata")
  ...
  dude<-c()
  for (clb in endgame$e2){
    clev<-metavDax[[1]][which(metavDax[[1]]$ev==clb),]
  ...
  dude2<-c()
  for (clb in endgame$e1){
    clev<-metavDax[[2]][which(metavDax[[2]]$ev==clb),]
  ...
  temsp<-aggregate(list(shit=spdude[,2]),
                  by = list(e1=spdude[,1],e2 = spdude[,3]),
                  FUN = function(x) c(length=length(unique(x))))
  temsp <- do.call(data.frame, temsp)

  endgame2<-na.omit(match(endgame$e2,temsp$e1))
...
tmp_array <- array(tmp_01, dim=c(nlon,nlat))
  ...
  scale_fill_gradientn(colours = rgb.palette(100))+
    geom_raster(data=oyoy,aes(x=Var1,y=Var2,fill=maxx,group=cluster),alpha=.9,interpolate = F)+
    geom_raster(data=verif,aes(x=Var1,y=Var2,fill=15,group=cluster),alpha=.9,interpolate = F)
