...
cocor3<-data.frame(compound,cocor2[,c(1,3,6,14,15,16,17)])

compwind<-na.omit(match(metaHax[[2]]$ev,compound$windevent))
compwind2<-na.omit(match(compound$windevent,metaHax[[2]]$ev))
cocow<-metaHax[[2]][compwind,]
cocow2<-metaHax[[2]][compwind2,]
cocow3<-data.frame(compound,cocow2[,c(1,8,11,14,15,16,17)])

compoundfinal<-data.frame(cocor3,cocow3[,c(5:11)])
...
stat_density_2d(data=metaHax[[1]],aes(x=lonMR,y=latMR,group=season,fill = stat(density)),geom = "raster", contour = FALSE,alpha=.6,n=150)
allprec<-metavHour[[2]]
allprec$season=1
...
polarEvent<-aggregate(metaHax[[1]]$season,
                 by = list(Month = metaHax[[1]]$x.month,Year=metaHax[[1]]$x.year),
...
ggplot(data=compoundfinal,aes(x=space,y=time,colour=wg.max,size=rf.max))+
  geom_point(alpha=.5)+
  theme(axis.text=element_text(size=16),
        axis.title=element_text(size=18,face="bold"),
        panel.background = element_rect(fill = "white", colour = "grey50"),
  ...
  scale_size(trans=scales::boxcox_trans(1.3),range=c(.4,17),"Max precipitation [mm]",breaks=c(20,60,120,Inf),limits=c(0,140),guide = guide_legend(override.aes = list(colour = alpha("grey",.8))))+
  scale_y_continuous(trans = log_trans(),
                     breaks = c(3,6,12,24,48,72,148),limits = c(0.9,150),"Temporal scale",
                     labels=c("3h","6h","12h","1day","2days","3days","1week"))+
  scale_x_continuous(trans = log_trans(),
                     breaks =c(1000,10000,100000,1000000)/450,limits=c(1,3000),"Spatial scale [km2]",
                     labels=c("1000","10000","100000","1000000")) 
