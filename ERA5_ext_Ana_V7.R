...
#1) Spatial compound
  ...
  SCompR<-inner_join(metatest,tesp,by=c("ev1","loc"))
  
  

  spSCR<-aggregate(list(rf=SCompR[,4],wg=SCompR[,5]),
                  by = list(ev2=SCompR[,10],ev1 = SCompR[,6],loc=SCompR[,9]),
  ...
  spSCR$cev=paste(spSCR$ev1,spSCR$ev2)
  for(rg in 1:length(spSCR2$ev2)){
    lesev<-paste(spSCR2[rg,2],spSCR2[rg,1])
    mr<-spSCR2$rf.max[rg]
    spSCR2$maxlocsR[rg]<-as.character(spSCR$loc[which(spSCR$cev==lesev & spSCR$rf.sum==mr)])
  }
#3) Spatiotemporal compound

  spkik<-sp03 %>% group_by(ev1,dd,loc) %>% mutate(id = row_number())
  sp20<-inner_join(metatest,spkik,by=c("ev1","loc","dd"))
  spSTCR<-sp20[which(sp20$id==1),]
  
  spSTCR2<-aggregate(list(rf=spSTCR[,4],wg=spSTCR[,5]),
                  by = list(ev2=spSTCR[,10],ev1 = spSTCR[,6],loc=spSTCR[,9]),
                  FUN = function(x) c(length=length((x)),max=max(x),sum=sum(x)))
...
names(compoundfinal)[c(30,31,37,38)]<-c("rf.space","rf.time","wg.space","wg.time")
compoundfinal$ORspace=compoundfinal$rf.space+compoundfinal$wg.space-compoundfinal$space
for (ti in 1:length(compoundfinal$windevent)){
compoundfinal$ORtime[ti]<-difftime(max(compoundfinal$endtimeRain[ti],compoundfinal$endtimeWind[ti]),min(compoundfinal$startimeRain[ti],compoundfinal$startimeWind[ti]),unit="hours")+1}

compoundfinal$ratiospace=compoundfinal$space/compoundfinal$ORspace
compoundfinal$ratiotime=compoundfinal$time/compoundfinal$ORtime
compoundfinal$t="rain"
compoundfinal$t[which(compoundfinal$wg.space>compoundfinal$rf.space)]="wind"

plot(compoundfinalT$rf.max.x,compoundfinalS$rf.max.x)
...
plot(compoundfinalST$)
maxspr<-maxrainc[-which(maxrainc[,2]==0),]
geodistc<-distGeo(compoundfinal[,c(20,21)],compoundfinal[,c(22,23)])/1000

geodistc2<-distGeo(maxrainc,maxwindc)/1000
geodistc2[which(geodistc2>2000)]=NA
plot(geodistc,geodistc2)

length(na.omit(geodistc2))
abline(a=0,b=1)
timedist=difftime(compoundfinal$maxtimeWind,compoundfinal$maxtimeRain,unit="hour")
timedist
geoshit<-data.frame(compoundfinal$combin, geodistc,geodistc2,maxrainc,maxwindc,compoundfinal$lonMR,compoundfinal$latMR,compoundfinal$lonMW,compoundfinal$latMW)



stdist<-data.frame(geodistc,as.numeric(timedist),as.character(compoundfinal$season))
names(stdist)<-c("geodistc","timedist","season")
stdist$season=as.character(stdist$season)

compoundfinal$season=as.character(compoundfinal$season)
ggplot(compoundfinal, aes(x=rf.max.a,fill=t,y=..density..)) + 
  geom_histogram(binwidth=2,color="black",position="identity", alpha=0.6)
...
m <- ggplot(compoundfinal, aes(x = ratiospace,y = ratiotime )) +
  geom_point() 
...
camembert<-aggregate(tabchoos$windevent,
...
#Seasonal analysis
bibcase<-compoundfinal[which(compoundfinal$t=="rain"),]
