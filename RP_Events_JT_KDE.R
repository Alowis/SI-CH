...
######hotspots###########################
    ...
    load(file="out/CompoundRW_79-19.ME2.Rdata")
    compound$meanday=yday(compound$startime)
    compound$angle=(compound$meanday*2*pi)/365
    xan=sum(cos(compound$angle))/length(compound$combin)
    yan=sum(sin(compound$angle))/length(compound$combin)
    rx<-sqrt(xan^2+yan^2)
    mmm<-(atan(yan/xan)+2*pi)*365/(2*pi)
    
    
    load(file="out/Wind_stfprint2.Rdata")
    load(file="out/Rain_stfprint.Rdata")
    sp03<-inner_join(nwo3,nvo3,by=c("loc","dd"))
    sp03<-sp03[order(sp03$ev.y),]
    sp03$combin=paste(sp03$ev.y,sp03$ev.x)
    idc<-unique(sp03$combin)
    idc
    View(idc)
    idcx<-seq(1:length(idc))
    idcxx<-data.frame(idc,idcx)
    olala<-match(sp03$combin,idcxx$idc)
    sp03$idc=idcxx$idcx[olala]
    cbm<-which(!is.na(match(sp03$combin,compoundX$combin)))
    sp04<-sp03[cbm,]
    tespx<-aggregate(list(len=sp04[,3]),
                     by = list(loc=sp04[,2]),
                     FUN = function(x) c(length=length(unique(x))))
    tespx <- do.call(data.frame, tespx)
    ...
    secu=c()
    for (lz in locaz){
    ucouthH<-sp03[which(sp03$loc==lz),]
    seq<-unique(ucouthH$idc)
    sequ<-paste(seq,collapse=" ")
    ucouthH$meanday=yday(ucouthH$dd)
    ucouthH$angle=(ucouthH$meanday*2*pi)/365
    xan=sum(cos(ucouthH$angle))/length(ucouthH$combin)
    yan=sum(sin(ucouthH$angle))/length(ucouthH$combin)
    rx<-sqrt(xan^2+yan^2)
    if(xan>0 & yan<0)mmm<-(atan(yan/xan)+2*pi)*365/(2*pi)
    if(xan>0 & yan>0)mmm<-(atan(yan/xan))*365/(2*pi)
    if(xan<0)mmm<-(atan(yan/xan)+pi)*365/(2*pi)
    
    rxt<-c(rxt,rx)
    dayof<-c(dayof,mmm)
    secu<-rbind(secu,sequ)
    
    }
    plot(dayof)
    rownames(secu)= tespx$loc
    secul=as.data.frame(secu)
    secul$local=tespx$loc
    names(secul)[1]="sequence"
    secul<-as.data.frame(secul)
    names(secul)=c("")
...
compoundX[which(compoundX$sgr==2),]
    
    
    
    validationdays<-read.csv(file=paste0(getwd(),"/csvs/ConfrontEvent1.csv"),header = T)
    # validationdays<-validationdays[-14,]
    validationdays[,1]<-as.Date(validationdays[,1],format='%d/%m/%Y')
    validationdays=validationdays[which(validationdays$Type=="Compound"),]
    ...
    ggplot(data=u2,aes(x=Var1,y=Var2,fill=omerd))+
      geom_point(size=4,shape=21,alpha=0.7)+
      geom_point(data=stconfF,aes(x=wg.max.cs,y=rf.max.cs,group=gr.y,color=factor(id)),shape=16,size=6,fill=NA,alpha=.4)+
      ...
      theme(axis.text=element_text(size=16),
            ...
            legend.key.size = unit(1, "cm"))+
      geom_line(data=inil, aes(x=X1,y=X2,fill=.01),size=2,color="lightblue4",alpha=.7)+
      scale_x_continuous(limits=c(17,50),"wa")+
      scale_y_continuous("ra") 
    
    length(unique(stconfF$id))
    diff(stconfF$startime)
      
     
    
    ggplot(data=compound,aes(x=wg.mean.cs,y=rf.mean.cs,fill=rp))+
      ...
      theme(axis.text=element_text(size=16),
            ...
            legend.key = element_rect(fill = "transparent", colour = "transparent"),
            legend.key.size = unit(1, "cm"))+
      scale_x_continuous(limits=c(15,50),"Wa") 
