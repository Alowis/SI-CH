...
# text(2e5,3.2,cex=1.8, labels=expression(paste( epsilon, " = 3.2")),pos=3,col="red")
# abline(h=3.2, col=2,lwd=3)
...
if(hazard==1)weightc=metav$vecmeta;epcl=3.2
if(hazard==2)weightc=metav$vecmeta2;epcl=2.5
if(hazard==3)weightc=rep(1,length(sampspd[,1]))

rpip<-dbscan(sampspd, eps=epcl, minPts = 5,weights = weightc)
...
library(viridis)
idiwant<-4
stplot<-stconfr[which(stconfS$id==idiwant),]
ggplot(uk_fort, aes(x=long,y=lat,group=group)) +
  geom_polygon(fill = "white", color = "gray10", size = 1) +
  theme_bw(16)+
  scale_color_viridis()+
  coord_fixed(xlim = longlims,  ylim = latlims, ratio = 1.3) +
  ylim(35,62)+
  geom_point(data=stplot,aes(x=lonMR,y=latMR,group=lonMR,color=len.max,size=rf.surf),alpha=0.8)+
  # geom_point(data=stplot,aes(x=X1.2,y=X2.2,group=X1.2,color=rf.max.a,size=spacescale),alpha=0.8)+
  scale_size(trans=scales::boxcox_trans(.9),range=c(.4,25))

   # geom_point(data=stplot,aes(x=X1.1.1,y=X2.1.1,group=X1.2,color=rf.max.a,size=2),alpha=0.8)
...
# stseason<-compoundfinal
idi=which(stseason$combin==stplot$combin[1])
...
idi=which(hazevent$ev==stplot$ev[4])
idmr<-hazevent$ev
idm1=idmr[idi]
# maxeventr<-metavHax[[1]][which(metavHax[[1]]$cluster==idm1 & metavHax[[1]]$time>=stseason$startime[idi] & metavHax[[1]]$time<=stseason$endtime[idi] ),]
maxeventw<-metavHax[[2]][which(metavHax[[2]]$cluster==idm2 & metavHax[[2]]$time>=stseason$startime[idi] & metavHax[[2]]$time<=stseason$endtime[idi] ),]

maxeventr<-olel[which(olel$ev1==idm1),]
...
tmp_array <- array(tmp_01a, dim=c(nlon,nlat))
  ...
  bs.palette=colorRampPalette(c("aliceblue","skyblue","dodgerblue","dodgerblue4"),interpolate="spline",bias=1)
  ...
  ggplot(uk_fort, aes(x=long,y=lat,group=group)) +
    ...
    scale_alpha_continuous(range = c(0.3, 0.7),trans="sqrt")+
