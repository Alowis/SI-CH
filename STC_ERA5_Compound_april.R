...
rm(nc,newformat,timestamp,crappy,alelu,file_time)
gc()
...
#
  ...
  if (hazard==1)eps=2.45
  if (hazard==2)eps=2.24
...
if(hazard==1)weightc=metav$vecmeta;epcl=2.45
if(hazard==2)weightc=metav$vecmeta2;epcl=2.24
...
gc()
max(metaHax[[1]]$rf)
hist(metaHax[[1]]$rf,breaks=70,xlab="# of points", main="# of points by rain cluster",col="skyblue",border="darkblue")
############################################
...
save(olel,file="allraininclustersNW.Rdata")
}else{load(file="allraininclusters.Rdata")}
...
# save(lonlatime,file="lonlatimeFAT.Rdata")



validationdays<-read.csv(file="ConfrontEvent1.csv",header = T)
# validationdays<-validationdays[-14,]
validationdays[,1]<-as.Date(validationdays[,1],format='%d/%m/%Y')
# validationdays[,2]<-as.Date(validationdays[,2],format='%d/%m/%Y')
stseason<-data.frame(compoundfinalST,compoundfinalS[,c(11:15)])
...
for (idiwant in 1:15 ){
  ...
  if (as.character(laval$Type)=="Compound") {mode="cp"}else{mode="sg"
  if (laval$Type=="Rain") hazevent<-metaHax[[1]]
  if (laval$Type=="Wind") hazevent<-metaHax[[2]]
  ...
  if (mode=="sg"){
  chosenones<-which(!is.na(match(hazevent$startimeD,validationdays[,1])))
  chosenones2<-which(!is.na(match(hazevent$endtimeD,validationdays[,1])))
  chosenonesF<-unique(c(chosenones,chosenones2))
  stconfr<-hazevent[chosenonesF,]
  }
  if (mode=="cp"){  
  chosenones<-which(!is.na(match(stseason$startimeD,validationdays[,1])))
  chosenones2<-which(!is.na(match(stseason$endtimeD,validationdays[,1])))
  chosenonesF<-unique(c(chosenones,chosenones2))
  stconfr<-stseason[chosenones,]
  }
  names(validationdays)[c(1)]=c("startimeD") 
  stconfS<-inner_join(stconfr,validationdays,by=c("startimeD"))
  names(validationdays)[c(1)]=c("endtimeD")
  stconfF<-inner_join(stconfr,validationdays,by=c("endtimeD"))
  stconfF<-stconfF[order(stconfF$id),]
  # stconfr2<-stconfr[,c(1:5,11,14,16:19,33:38,49:55)]
  # stconfr2$idc<-seq(1:length(stconfr2$combin))
  
  
  
  if (mode=="sg"){
  idev<-c(stconfS$ev[which(stconfS$id==idiwant)],stconfF$ev[which(stconfF$id==idiwant)])
  idev<-unique(idev)
  stplot<-stconfr[which(!is.na(match(stconfr$ev,idev))),]
  }
  if (mode=="cp"){
    idev<-c(stconfS$combin[which(stconfS$id==idiwant)],stconfF$combin[which(stconfF$id==idiwant)])
    idev<-unique(idev)
    stplot<-stconfr[which(!is.na(match(stconfr$combin,idev))),]
  }
  stplot<-stplot[order(stplot$startimeD),]
  
  if(mode=="cp"){
    names(stplot)[c(39,40,36,35)]<-c("lon","lat","inter","space")
  }
  if(mode=="sg" & laval$Type=="Rain"){
    names(stplot)[c(20,21,27,11)]<-c("lon","lat","inter","space")
  }
  if(mode=="sg" & laval$Type=="Wind"){
    names(stplot)[c(22,23,8,11)]<-c("lon","lat","inter","space")
  }
  ggplot(uk_fort, aes(x=long,y=lat,group=group)) +
    geom_polygon(fill = "white", color = "gray10", size = 1) +
    theme_bw(16)+
    ...
    coord_fixed(xlim = longlims,  ylim = latlims, ratio = 1.3) +
    ylim(35,62)+
    geom_point(data=stplot,aes(x=lon,y=lat,group=lon,color=inter,size=space),alpha=0.8)
  
  stclust<-c(stclust,length(stplot[,1]))
  tipi<-c(tipi,as.character(laval$Type[1]))
  }
  
stclust
tipi
loopix=F
rescomp1<-data.frame(stclust,tipi)
...
if(mode=="cp"){
  if (loopix==T){
    maxeventrx<-c()
    maxeventwx<-c()
    for (ix in 1:length(stplot$combin)){
      idi=which(stseason$combin==stplot$combin[ix])
      idmr<-stseason$ev1
      idmw<-stseason$ev2
      print(idi)
      idm1=idmr[idi]
      idm2=idmw[idi]
      maxeventrx1<-olel[which(olel$ev1==idm1),]
      maxeventrx1$gr=idm1
      maxeventrx<-rbind(maxeventrx,maxeventrx1)
      
      maxeventwx1<-metavHax[[2]][which(metavHax[[2]]$cluster==idm2),]
      maxeventwx1$gr=idm2
      maxeventwx<-rbind(maxeventwx,maxeventwx1)
      
    }
    lonlatrx=matrix(as.numeric(unlist(strsplit(maxeventrx$loc," "))),ncol=2,byrow = T)
    maxeventrx<-data.frame(lonlatrx,maxeventrx)
    maxeventrx$time <- as_datetime(c(maxeventrx$Var3*60*60),origin="1900-01-01") 
    maxeventwx$time <- as_datetime(c(maxeventwx$Var3*60*60),origin="1900-01-01")
  }else{
  idi=which(stseason$combin==stplot$combin[3])
  ...
  if (looprain==T){
    maxeventr<-c()
    for(ir in 1:length(stplot$ev1)){
      idi=which(stseason$ev1==stplot$ev1[ir])
      idmr<-stseason$ev1
      idm1=idmr[idi]
      
      maxeventrp<-olel[which(olel$ev1==idm1),]
      maxeventr<-rbind(maxeventr,maxeventrp)
    }
   }
  }
}
...
if (looprain==T){
maxeventr<-c()
for(ir in 1:length(stplot$ev)){
idi=which(hazevent$ev==stplot$ev[ir])
idmr<-hazevent$ev
idm1=idmr[idi]
...
maxeventr<-rbind(maxeventr,maxeventrp)
  ...
  #for rain only
  idi=which(hazevent$ev==stplot$ev[1])
  idmr<-hazevent$ev
  idm1=idmr[idi]
  #for wind only
  idi=which(hazevent$ev==stplot$ev[1])
...
ggplot(uk_fort, aes(x=long,y=lat,group=group)) +
  ...
  coord_fixed(xlim = longlims,  ylim = latlims, ratio = 1.3) +
  geom_encircle(aes(x=X1, y=X2,group=gr),
                data = maxeventrx, size = 2, color = "blue",s_shape=.8, expand=0,fill="blue",alpha=.5)+
...
lonlatr=matrix(as.numeric(unlist(strsplit(maxeventr$loc," "))),ncol=2,byrow = T)
maxeventr<-data.frame(lonlatr,maxeventr)
temp01<- hazmat[[1]]$data[ , ,1]
...
maxevsum<-sum(maxeventr$arain.yoboys.[which(tmp_02a==mx)])
mxs<-c(mxs,maxevsum)
}
...
tmp_array <- array(tmp_01a, dim=c(nlon,nlat))
  ...
  ouh<-data.frame(lonlat[ohcrap,],0,0,0,0,tmp_07a$time[1],0,0)
  ...
  names(elmaxoua)[4]="cloc"
  elmaxouc<-inner_join(elmaxoua,elmaxoub,by="cloc")
  
  
  bs.palette=colorRampPalette(c("skyblue","dodgerblue","blue","darkblue","black"),interpolate="spline",bias=1)
  bs.palette2=colorRampPalette(c("lightgreen","yellowgreen","gold","orangered","deeppink","white"),interpolate="spline",bias=1.5)
  bs.palette3=colorRampPalette(c("orange","blue","purple"),interpolate="linear",bias=1)
  valrain<-seq(10,1000,length=10)
  valwind=seq(20,50,b=5)

  library(ggalt)
  elmaxoua$X1=elmaxoua$X1
  
  ggplot(uk_fort, aes(x=long,y=lat,group=group)) +
    theme_bw(16)+
    geom_raster(data=elmaxoub,aes(x=Var1,y=Var2,fill=mwg,group=cluster,alpha=mwg/max(mwg)),interpolate = F)+
    ...
    geom_polygon(fill = "transparent", color = "gray10", size = 1) +
    coord_fixed(xlim = longlims,  ylim = latlims, ratio = 1.3) +
    scale_fill_gradientn(trans=scales::modulus_trans(.5),colors=bs.palette2(100),"w a [m/s]",breaks=c(20,30,40,50,Inf),limits=c(15,51))+
    new_scale_fill() +
    geom_raster(data=elmaxoua,aes(x=X1,y=X2,fill=maxx,group=ev1,alpha=maxx/max(maxx)),interpolate = F)+
    # geom_encircle(aes(x=X1, y=X2,group=ev1),
    #               data = maxeventr, size = 2, color = "blue",s_shape=.8, expand=0,fill="blue",alpha=.5)+
    # geom_encircle(aes(x=Var1, y=Var2,group=cluster),
    #               data = maxeventw, size = 2, color = "orange",s_shape=.8, expand=0,fill="orange",alpha=.5)+
    # geom_contour_fill(data=elmaxoua1,aes(x=Var1,y=Var2,z=maxx,group=cluster),alpha=0.3,na.fill=T)+
    scale_alpha_continuous(range = c(0.3, 0.7),guide = FALSE)+
      scale_fill_gradientn(trans=scales::modulus_trans(.5),colors=bs.palette(100),"r a [m/s]",breaks=c(5,10,25,50,75,Inf),limits=c(2,110))+
  theme(axis.text=element_text(size=16),
        axis.title=element_text(size=18,face="italic"),
        panel.background = element_rect(fill = "white", colour = "grey50"),
        legend.title = element_text(size=18),
  ...
  mr<-max(elmaxoua$maxx)
  length(elmaxoua$X1)
  elmaxoua$loc[which(elmaxoua$maxx==mr)]
  max(elmaxoub$mwg)
    ggplot(uk_fort, aes(x=long,y=lat,group=group)) +
      theme_bw(16)+
      geom_polygon(fill = "transparent", color = "gray10", size = 1) +
      ...
      geom_raster(data=elmaxoua,aes(x=X1,y=X2,fill="1",group=ev1),alpha=0.7,interpolate = F)+
      geom_raster(data=elmaxouc,aes(x=X1,y=X2,fill="3",group=ev1),alpha=0.7,interpolate = F)+
      scale_fill_manual(values = c( "1"="blue", "2" ="orange","3"="green"),"Hazard footprint",labels = c("rain", "wind", "compound"))
    ...
    rsplot<-read.csv(file="dataconf1.csv",header = T)
    rsplot<-data.frame(rsplot,rescomp1)
    rsgbar<-data.frame(rep(as.character(rsplot$Name),3))
    rsgbar$var<-c(rep("1ra",15),rep("2wa",15),rep("3ts",15))
    rsgbar$val<-c(rsplot$rel.ra,rsplot$rel.wa,rsplot$rel.ts)
    rsgbar$val=rsgbar$val*100
    rsgbar$id<-rep(rsplot$id,3)
    names(rsgbar)[1]= "Eventname"
    lesmean<-100*c(mean(rsplot$rel.ra,na.rm=T),mean(rsplot$rel.wa,na.rm=T),mean(rsplot$rel.ts))
    
    ggplot(rsgbar,aes(x=id, y=val,group=var))+geom_bar(stat = "identity", aes(fill=factor(var)),color="black",size=.4,position = "dodge",alpha=.7) +
      scale_fill_manual(values = c( "blue","orange","grey"),labels= c(expression('r'["a"]),expression('w'["a"]),"duration")) +
      scale_x_continuous(breaks=rsplot$id,labels=rsplot$Name)+
      scale_y_continuous(breaks=seq(-80,60, by=20))+
      labs(y = "relative error (%)",x = "Events",colour = "Parameter",fill= "attributes",size=10)+
      ...
      theme(axis.text=element_text(size=12),
            ...
            legend.text.align = 0,
            legend.key.size = unit(1, "cm"))+
      geom_hline(yintercept=lesmean[1], color="blue",linetype="dashed",size=1.2,alpha=.7)+
      geom_hline(yintercept=lesmean[2], color="orange",linetype="dashed",size=1.2,alpha=.7)+
      geom_hline(yintercept=lesmean[3], color="grey",linetype="dashed",size=1.2,alpha=.7)
      ...
      confres<-read.csv(file="dataconf1.csv",header = T)
      plot(confres$obs.wa,confres$mod.wa,xlim=c(20,60),ylim=c(20,60))
      abline(a=0,b=1)
      plot(metaHax[[2]]$x.dur+rnorm(length(metaHax[[2]]$ev), sd=0.5),metaHax[[2]]$wg.max,log="x")
      points(stseason$ORtime+rnorm(length(stseason$combin),sd=0.5),stseason$wg.max.c,col=2)
  boxplot(wg.time~x.month,data=stseason)
  
  
  tday<-seq(1:365)
  tdayt<-seq(as.Date("2009-01-01"),as.Date("2018-01-01"),"days")
  stseason$daysevent<-yday(compoundfinalST$startime)
  pday<-aggregate(stseason$combin ,
                      by = list(day = stseason$startimeD),
                      FUN = function(x) c(len = length(na.omit(x))))
  pday <- do.call(data.frame, pday)
  interdays<-tdayt[which(is.na(match(tdayt,pday$day)))]
  zday<-data.frame(interdays,rep(0,length(interdays)))
  
  names(zday)=names(pday)
  pday<-rbind.data.frame(pday,zday)
  plot(pday[,1],pday[,2])
  pdays<-pday[order(pday$day),]
  library(zoo)
  rmean<-rollmean(pdays$x,30)
  plot(rmean,type="o")
  
  
  qday<-aggregate(stseason$combin ,
                  by = list(day = stseason$daysevent),
                  FUN = function(x) c(len = length(na.omit(x))))
  qday <- do.call(data.frame, qday)
  
  interdays<-tday[which(is.na(match(tday,qday$day)))]
  zday<-data.frame(interdays,rep(0,length(interdays)))
  
  names(zday)=names(pday)
  qday<-rbind.data.frame(qday,zday)
  qdays<-qday[order(qday$day),]
  rsum<-rollsum(qdays$x,31)
  plot(rsum,type="o")
  
  
  losbol<-list()
  lengou<-c()
  kek<-matrix(nrow=length(tday),ncol=1)
  msi<-matrix(nrow=length(tday),ncol=1)
  for (dday in 1:length(tday)){
    cheese<-seq(dday-15,dday+15)
    if(length(which(cheese<=0))>0){
      cheese[which(cheese<=0)]=cheese[which(cheese<=0)]+365
    }
    if(length(which(cheese>365))>0){
      cheese[which(cheese>365)]=cheese[which(cheese>365)]-365
    }
    losbol[[dday]]<-stseason[which(!is.na(match(stseason$daysevent,cheese))),]
    lengou[dday]<-length(losbol[[dday]]$combin)
    dtch<-data.frame(losbol[[dday]]$rf.max.c,losbol[[dday]]$wg.max.c)
    ms<-mean(losbol[[dday]]$spacescale)
    msi[dday]<-ms
    # ql=c(.5,.9)
    # ki<-chi(dtch,qlim=ql,nq=100)$chi
    ki<-cor.test(dtch[,1],dtch[,2],method="kendall")$estimate
    kek[dday]=ki
    # ken<-cor.test(dtch[,1],dtch[,2],method="kendall")
  }
  plot(lengou,type="h",col="blue") 
  plot(msi,type="h",col="darkblue")
  lengou<-as.data.frame(lengou)
  lengou$day=tday
  ...
  p <- ggplot() + 
    geom_segment2(data=lengou,aes(x=day, 
                     xend=day, 
                     y=0, 
                     yend=lengou),color="blue")+ theme_bw()
  p +coord_radar()
  
  
  dtch<-data.frame(losbol[[200]]$rf.max.c,losbol[[200]]$wg.max.c)
  ki<-chi(dtch)$chi
  ken<-cor.test(dtch[,1],dtch[,2],method="kendall")
  plot(ki[,2])
  tevent<-compoundfinalST$startime[order(compoundfinalST$startime)]
  difftime(tevent,units = hours)
  uevent<-as.matrix(compoundfinalST$endtime)
  tuevent<-cbind(tevent,uevent)
  daysevents<-data.frame(compoundfinalST$combin,daysevent)
  seq<-dist(daysevent)
  dt<-diff(tuevent)


  mimi<-match(mierda,easypizy)
  easypizy<-cbind(easypizy)
  plot(easypizy)
  abline(v=c(365,730,1000))
  didi<-dist(easypizy)
  hist(daysevent,breaks=50)
  reservo <- optics(oualaw,eps=10, minPts = 5)
  res=extractDBSCAN(reservo, eps_cl =2)
  plot(res)
  res <- extractXi(res, xi = 0.1)
  res
  points(easypizy,col=res$cluster,pch=4)
